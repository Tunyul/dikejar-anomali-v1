...
static func has_ready_to_claim_missions_in_save(save_path: String = "user://save.cfg") -> bool:
    ...
    for m_any in missions_list:
        ...
        if mt != "challenge":
            if claimed.has(id_str) and bool(claimed[id_str]):
                continue
